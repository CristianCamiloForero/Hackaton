...
p = Path(__file__).parent.parent / 'simulate_response.json'
if not p.exists():
    p = Path('simulate_response.json')
    if not p.exists():
        raise SystemExit('No se encontró simulate_response.json en el repo ni en el working dir')
